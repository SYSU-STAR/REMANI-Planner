...
ros::Subscriber odom_sub, map_state_sub_;
...
void mapStateCallBack(const std_msgs::Int32::ConstPtr &msg){
...
void rcvGlobalPointCloudCallBack(const sensor_msgs::PointCloud2& pointcloud_map) {
  ...
  // ROS_WARN("New Global Pointcloud received..");

  pcl::PointCloud<pcl::PointXYZ> cloud_input;
  pcl::fromROSMsg(pointcloud_map, cloud_input);

  // _voxel_sampler.setLeafSize(0.1f, 0.1f, 0.1f);
  _voxel_sampler.setLeafSize(_resolution, _resolution, _resolution);
...
void renderSensedPoints(const ros::TimerEvent& event) {
  ...
  if (_kdtreeLocalMap.radiusSearch(searchPoint, sensing_horizon,
    ...
    for (size_t i = 0; i < _pointIdxRadiusSearch.size(); ++i) {
      ...
      //     tan(M_PI / 12.0))
      //   continue;
      // if ((fabs(pt.z - _odom.pose.pose.position.z) / sensing_horizon) > tan(M_PI / 3.0))
      //   continue; 
...
int main(int argc, char** argv) {
  ...
  map_state_sub_ = nh.subscribe("/map_generator/map_state", 10, mapStateCallBack);
